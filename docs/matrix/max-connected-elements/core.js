 ...
 * ====== START DUAL-WAVE PROCESS ======
 * Searching wave scans grid, infection wave floods components
 * runs through main diagonal to look for any cell of connected zone
 */
...
function fasterInfectionWave() {

    //looks for the first occurence of one in every row 
    for (let i = 0, j = 0; i < rows && j < rows; i++, j++) {
        if (map[i][j] !== 1) { continue } else {
            //find connected
...
function fastestInfectionWave() {

    //loops through the mian diagonal
    for (let i = 0, j = 0; i < rows && j < rows; i++, j++) {
        if (map[i][j] !== 1) { continue } else {
